...
'''
async def get_objects_by_ids(ids: list[str]):
    query = "&".join([f"id={i}" for i in ids])
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{BASE_URL}?{query}")
        print("get_objects_by_ids",resp.json())
...
call_mcp_server_agent = LlmAgent(
    ...
    When the user asks to fetch data for a specific object ID, use the `get_objects_by_ids` tool and pass the ID to it.
    """,
    tools=[get_objects_by_ids],
)
...
@mcp.tool()
async def get_objects_by_ids_using_adk_agent(ctx: Context = None):
    final_result = await get_agent_async("Fetch the data for object_id 2, pass the id to get_objects_by_ids tool")
    return final_result
...
if __name__ == "__main__":
    ...
    #mcp.run(transport="sse", host="127.0.0.1", port=8001, path="/mcp")
